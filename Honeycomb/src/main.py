...
from lvl4_freeortrapped import lvl4_freeOrTrapped, find_w_position
from lvl5_freeOrTrapped import evaluate_and_place_obstacles,print_hexagonal_grid

def is_active(level, target_level):
...
if __name__ == '__main__':
    ...
    # Define levels and their states
    levels = {"level1": False, "level2": False, "level3": False, "level4": False, "level5": True}

    # Determine active level
    active_level = next((level for level, active in levels.items() if active), None)

    if active_level:
        input_location = current_dir / f"../Inputs/{active_level}"
        output_location = current_dir / f"../Outputs/{active_level}"
    else:
        raise ValueError("No active level selected.")

    print("Input location:", input_location.resolve())
    print("Output location:", output_location.resolve())
    ...
    for f_p in input_files:
        ...
        if active_level == "level4":
            ...
            for grid in combs:
                ...
                # Determine if 'W' is free or trapped
                result = lvl4_freeOrTrapped(grid, w_row, w_col, verbose=verbose)
                results.append(result)
