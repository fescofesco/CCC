...
def solve_level1(input_text):
    """
    Solve Level 1 problem: sum paces for each sequence
    ...
    # Next N lines: sequences of paces
    for i in range(1, N + 1):
        # Split the line into integers
        parts = lines[i].split()
        paces = [int(x) for x in parts]
        
        # Total time is just the sum of paces
...
def test_with_files():
    ...
    level1_dir = r"c:\Users\accou\Code\Coding_Contest\CCC\Challenge 2025\Johannes\LEVEL1\level1"
    
    # Find all .in files
    in_files = [f for f in os.listdir(level1_dir) if f.endswith('.in')]
    
    for in_file in in_files:
        print(f"Processing {in_file}...")
        
        # Read input file
        in_path = os.path.join(level1_dir, in_file)
        with open(in_path, 'r') as f:
            input_text = f.read()
        
        # Solve the problem
        output_text = solve_level1(input_text)
        
        # Write output file
        out_file = in_file.replace('.in', '_generated.out')
        out_path = os.path.join(level1_dir, out_file)
        ...
        expected_out_path = os.path.join(level1_dir, expected_out_file)
