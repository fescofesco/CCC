...
def pad_sequences_to_avoid_asteroids(x_seq, y_seq, asteroids, target_x, target_y, time_limit):
    ...
    if grid_path:
        ...
        print(f"    No grid path found, using fallback strategies...")
    
    # Fallback: try simple heuristic strategies if grid path finding didn't work
    from validator_level7 import simulate_movement, check_asteroid_collisions
    
    def test_sequences(x_test, y_test):
        """Test if sequences are valid"""
    ...
    # Strategy 0: Try original sequences
    ...
    # Strategy 1: Delay X (move Y first)
    ...
    # Strategy 2: Delay Y (move X first)
    for delay in range(1, 200):
        y_test = [0] * delay + y_seq
    ...
    # Strategy 3: Delay BOTH dimensions (sampled combinations - not all)
    # Sample delays more sparsely for efficiency
    delay_samples = [1, 2, 3, 5, 10, 15, 20, 30, 40, 50, 75, 100]
    for x_delay in delay_samples:
        for y_delay in delay_samples:
            if x_delay >= 200 or y_delay >= 200:
                continue
            x_test = [0] * x_delay + x_seq
            y_test = [0] * y_delay + y_seq
            max_len = max(len(x_test), len(y_test))
            x_padded = x_test + [0] * (max_len - len(x_test))
            y_padded = y_test + [0] * (max_len - len(y_test))
    ...
    # Strategy 4: Add delays in the MIDDLE of sequences (sampled)
    mid_delay_samples = [1, 2, 3, 5, 10, 20, 30, 50]
    for mid_delay in mid_delay_samples:
        # Insert delay in middle of X sequence
        mid_idx = len(x_seq) // 2
        x_test = x_seq[:mid_idx] + [0] * mid_delay + x_seq[mid_idx:]
        max_len = max(len(x_test), len(y_seq))
        x_padded = x_test + [0] * (max_len - len(x_test))
        y_padded = y_seq + [0] * (max_len - len(y_seq))
        ...
        # Insert delay in middle of Y sequence
        mid_idx = len(y_seq) // 2
        y_test = y_seq[:mid_idx] + [0] * mid_delay + y_seq[mid_idx:]
        max_len = max(len(x_seq), len(y_test))
        x_padded = x_seq + [0] * (max_len - len(x_seq))
        y_padded = y_test + [0] * (max_len - len(y_test))
    ...
    # Strategy 5: X detours (go past target, wait, return) - reduced search
    detour_samples = [1, 2, 3, 5, 10, 15, 20]
    wait_samples = [0, 1, 2, 3, 5, 10, 20, 30, 50]
    for extra in detour_samples:
        for direction in [1, -1]:
            detour_target = target_x + extra * direction
            try:
                x_detour = generate_sequence(detour_target, time_limit * 2)
            except:
                continue
            ...
            # Try different wait times (sampled)
            for y_wait in wait_samples:
    ...
    # Strategy 6: Y detours (go past target, wait, return) - reduced search
    for extra in detour_samples:
        for direction in [1, -1]:
            detour_target = target_y + extra * direction
            try:
                y_detour = generate_sequence(detour_target, time_limit * 2)
            except:
                continue
            ...
            # Try different wait times (sampled)
            for x_wait in wait_samples:
                y_combined = y_detour[:-1] + [0] * (len(x_seq) + x_wait - 2) + y_return[1:]
                x_combined = [0] * (len(y_detour) - 1) + [0] * x_wait + x_seq + [0] * (len(x_return) - 2)
    ...
    # Strategy 7: BOTH X and Y detours (larger loop around asteroids) - reduced search
    both_detour_samples = [1, 2, 3, 5, 10]
    for x_extra in both_detour_samples:
        for x_dir in [1, -1]:
            for y_extra in both_detour_samples:
                for y_dir in [1, -1]:
                    try:
                        # X detour
                        x_detour_target = target_x + x_extra * x_dir
                        x_detour = generate_sequence(x_detour_target, time_limit * 2)
                        x_return_dist = abs(x_detour_target - target_x)
                        x_return_dir = -1 if x_detour_target > target_x else 1
                        x_return = [0] + [5 * x_return_dir] * x_return_dist + [0]
                        
                        # Y detour
                        y_detour_target = target_y + y_extra * y_dir
                        y_detour = generate_sequence(y_detour_target, time_limit * 2)
                        y_return_dist = abs(y_detour_target - target_y)
                        y_return_dir = -1 if y_detour_target > target_y else 1
                        y_return = [0] + [5 * y_return_dir] * y_return_dist + [0]
                        
                        # Combine: go out, return
                        x_combined = x_detour[:-1] + x_return[1:]
                        y_combined = y_detour[:-1] + y_return[1:]
    ...
    raise ValueError(f"Cannot find collision-free path to ({target_x},{target_y}) avoiding {len(asteroids)} asteroids")
